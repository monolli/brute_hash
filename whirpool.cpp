...
ifstream input("pss2.txt");
string line,str=" ";
char try1[8];
int tam=0;
time_t start,end;
CryptoPP::Whirlpool wrpl;
								...
								for(int k=32;k<127;k++){							//go through characters in the 3rd position
									...
									for(int j=32;j<127;j++){						//go through characters in the 2nd position
										...
										for(int i=32;i<127;i++){								//go through characters in the 1st position
												...
												//cout<<str<<endl;
												string hash;
												CryptoPP::StringSource (str, true, new CryptoPP::HashFilter(wrpl, new CryptoPP::HexEncoder(new CryptoPP::StringSink(hash))));
												//cout<<hash<<endl;
